...
export async function createMonitor(
  ...
  // Lowercase address and createdBy for consistency
  const lowercasedAddress = address?.toLowerCase();
  try {
    // get webhook from nodit API
    const getWebhookRes = await fetch(
      `${BASE_URL}/ethereum/sepolia/webhooks?subscriptionId=${NODIT_WEBHOOK_ID}`,
      {
        headers: {
    ...
    // Combine addresses from monitor and webhook
    let updatedAddresses;
    if (isActive) {
      // Add address if monitor is active
      updatedAddresses = [lowercasedAddress, ...addressesFromWebhook];
    } else {
      // Remove address if monitor is not active
      updatedAddresses = addressesFromWebhook.filter(
        (addr) => addr !== lowercasedAddress
      );
    }

    // deduplicate addresses before updating webhook
    updatedAddresses = Array.from(
      new Set(updatedAddresses.map((addr) => addr.toLowerCase()))
    );
    ...
    const updateWebhookRes = await fetch(
      `${BASE_URL}/ethereum/sepolia/webhooks/${NODIT_WEBHOOK_ID}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  try {
    ...
    if (monitor) {
      ...
      const webhookRes = await fetch(
        `${BASE_URL}/ethereum/sepolia/webhooks?subscriptionId=${monitor.webhookId}`,
        {
          headers: {
            "Content-Type": "application/json",
            "X-API-Key": process.env.NODIT_API_KEY
      ...
      const updatedAddresses = addressesFromWebhook.filter(
        (address) => address !== monitor.address
      );

      // deduplicate addresses before updating webhook
      const updatedUniqueAddresses = Array.from(
        new Set(updatedAddresses.map((addr) => addr.toLowerCase()))
      );
      // Update webhook via Nodit API to remove addresses
      const updateWebhookRes = await fetch(
        `${BASE_URL}/ethereum/sepolia/webhooks/${monitor.webhookId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            condition: {
              addresses: updatedUniqueAddresses // If no addresses left, we might want to delete the webhook instead
            }
          })
        }
      );
