...
export async function createMonitor({ email, addresses }) {
  ...
  try {
    ...
    const getWebhookRes = await fetch(
      `https://web3.nodit.io/v1/ethereum/sepolia/webhooks?subscriptionId=${NODIT_WEBHOOK_ID}`,
      {
        headers: {
    ...
    const addressesFromWebhook = webhookData.condition.addresses || [];
    ...
    const updateWebhookRes = await fetch(
      `https://web3.nodit.io/v1/ethereum/sepolia/webhooks/${NODIT_WEBHOOK_ID}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  try {
    ...
    if (monitor) {
      ...
      const webhookRes = await fetch(
        `https://web3.nodit.io/v1/ethereum/sepolia/webhooks?subscriptionId=${monitor.webhookId}`,
        {
          headers: {
            "Content-Type": "application/json",
            "X-API-Key": process.env.NODIT_API_KEY
      ...
      const addressesFromWebhook = webhook.condition.addresses || [];
